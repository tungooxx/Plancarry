#!/usr/bin/env python3
"""Registered-identity adapter for ReplayResidual V2.2 packet production.
...
SUCCESSOR_PACKET_REL=Path('results/science/plancarry_replay_residual_sanity_packets_v2_2_technical_retry1')
_ALLOWED_STAGE1={
...
def _successor_meta()->dict[str,Any]:
 i=_bound_info()
 return {
  'template_sha256':TEMPLATE_SHA256,'bound_contract_sha256':i['bound_contract_sha256'],
  'successor_experiment_id':i['experiment_id'],'successor_prediction_id':i['prediction_id'],
  'materialization_review_sha256':i['materialization_review_sha256'],
  'successor_policy_sha256':SUCCESSOR_POLICY_SHA256,'successor_policy_review_sha256':SUCCESSOR_POLICY_REVIEW_SHA256,
  'execution_attestation_contract_sha256':ATTESTATION_CONTRACT_SHA256,'execution_attestation_review_sha256':ATTESTATION_REVIEW_SHA256,
...
def _rewrite_hidden_identity(hidden:Path,old_manifest:dict[str,Any])->dict[str,Any]:
 ...
 manifest['kind']='REPLAY_RESIDUAL_V2_2_TECHNICAL_SUCCESSOR_PACKET_SET_MANIFEST'
 manifest['experiment_id']=info['experiment_id']; manifest['prediction_id']=info['prediction_id']
 manifest['technical_successor_v2_2']=meta
 expected=copy.deepcopy(old_manifest); expected['kind']=manifest['kind']; expected['experiment_id']=info['experiment_id']; expected['prediction_id']=info['prediction_id']; expected['technical_successor_v2_2']=meta
 if manifest!=expected: raise RuntimeError('MANIFEST_REBIND_DRIFT')
 _replace_json(mp,manifest)
 provenance=copy.deepcopy(old_prov)
 provenance['kind']='REPLAY_RESIDUAL_V2_2_TECHNICAL_SUCCESSOR_PACKET_SET_PROVENANCE'
 provenance['packet_manifest_sha256']=frozen.sha256_file(mp)
 provenance['technical_successor_v2_2']=meta
 expectedp=copy.deepcopy(old_prov); expectedp['kind']=provenance['kind']; expectedp['packet_manifest_sha256']=provenance['packet_manifest_sha256']; expectedp['technical_successor_v2_2']=meta
...
def _validate_successor_dir(directory:Path)->dict[str,Any]:
 ...
 if m.get('kind')!='REPLAY_RESIDUAL_V2_2_TECHNICAL_SUCCESSOR_PACKET_SET_MANIFEST': raise RuntimeError('SUCCESSOR_MANIFEST_KIND_MISMATCH')
 ...
 if m.get('technical_successor_v2_2')!=meta: raise RuntimeError('SUCCESSOR_MANIFEST_META_MISMATCH')
 ...
 if p.get('packet_manifest_sha256')!=frozen.sha256_file(d/'manifest.json') or p.get('technical_successor_v2_2')!=meta: raise RuntimeError('SUCCESSOR_PROVENANCE_MISMATCH')
...
def attest(directory:Path,result_path:Path|None,output:Path)->int:
 scan=scan_technical_errors(directory); info=_bound_info(); base={
  'kind':'PLANCARRY_REPLAYRESIDUAL_V22_EXECUTION_ATTESTATION','attestation_contract_sha256':ATTESTATION_CONTRACT_SHA256,
