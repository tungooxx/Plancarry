...
class BridgeApplication:
    def __init__(self, backend: Any, token: str, disable_patch: bool = False):
        if not token:
            raise ValueError("auth token must be non-empty")
        self.backend = backend
        self.token = token.encode("utf-8")
        self.disable_patch = bool(disable_patch)
        self.allowed_post = ALLOWED_POST - ({"/patch_score"} if self.disable_patch else set())
...
def make_handler(app: BridgeApplication):
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:  # noqa: N802
            if self.path not in app.allowed_post:
                self._json(HTTPStatus.NOT_FOUND, {"ok": False, "error": {"code": "NOT_FOUND", "message": "unknown endpoint"}})
                return
            if not self._auth():
                return
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--disable-patch", action="store_true", help="disable /patch_score for capture-only phase isolation")
...
def main() -> int:
    ...
    if args.mock:
        ...
        try:
            ...
            return 2
    app = BridgeApplication(backend, token, disable_patch=args.disable_patch)
