#!/usr/bin/env python3
"""Reviewed PlanCarry-Latent v2.5 discovery/confirmation runner.
...
from plancarry_latent_v2_5_validator import (
    DONOR_MAP_SHA256,
    MANIFEST_SHA256,
    PREREG_SHA256,
    REVIEW_SHA256,
...
def run_discovery(client: ClientProtocol, output: Path) -> str:
    ...
    if len(competent_indices) < 12:
        result = {"kind":"PLANCARRY_LATENT_V2_5_DISCOVERY", "status":"INCONCLUSIVE_MODEL_EXPRESSIVITY",
    ...
    selection={
        "kind":"PLANCARRY_LATENT_V2_5_DISCOVERY_SELECTION",
...
def run_confirmation(client: ClientProtocol, selection_path: Path, selection_sha256: str, output: Path) -> str:
    ...
    if overall<16 or delayed<8:
        result={"kind":"PLANCARRY_LATENT_V2_5_CONFIRMATION","status":"INCONCLUSIVE_MODEL_EXPRESSIVITY",
    ...
    result={
        "kind":"PLANCARRY_LATENT_V2_5_CONFIRMATION","status":decision["status"],
