#!/usr/bin/env python3
"""Reviewed PlanCarry-Latent v2.6 RTX3050 runtime-only discovery/confirmation runner.
...
live bridge calls require exact RTX3050/model/runtime provenance.
"""
...
from plancarry_latent_v2_6_validator import (
    DONOR_MAP_SHA256,
    MANIFEST_SHA256,
    PREREG_SHA256,
    REVIEW_SHA256,
    DESIGN_SHA256,
    REGISTRATION_WRAPPER_SHA256,
    REGISTRATION_WRAPPER_AUDIT_SHA256,
    SUPERSESSION_AUDIT_SHA256,
    DIRECT_VS_CACHE_SHA256,
    PREFIX_STABLE_BRIDGE_SHA256,
    WHITEBOX_CLIENT_SHA256,
    PREFIX_STABLE_ENGINEERING_RESULT_ID,
    V26_RUNTIME_REBIND_SHA256,
    V26_RUNTIME_REBIND_REVIEW_SHA256,
    V26_REGISTRATION_WRAPPER_SHA256,
    V26_REGISTRATION_WRAPPER_AUDIT_SHA256,
    V26_EXPECTED_DEVICE_NAME,
    V26_EXPECTED_TORCH,
    EXPERIMENT_ID,
    PREDICTION_ID,
...
class PairContext:
    ...
    def __post_init__(self) -> None:
        for name, text in [("reset_block", self.reset_block), ("active_a", self.active_a),
            ...
            if "\\n" in text:
                raise RuntimeError(f"V25_LITERAL_BACKSLASH_N_FORBIDDEN {name}")
            if not text.endswith("<STATE_END>\n") or text.endswith("<STATE_END>\n\n"):
                raise RuntimeError(f"V25_TERMINAL_NEWLINE_CONTRACT_FAILED {name}")
...
def build_pair_context(pair: dict[str, Any], prereg: dict[str, Any]) -> PairContext:
    ...
    if reset_block != pair_reset:
        raise RuntimeError(f"V25_RESET_TEMPLATE_DIVERGENCE pair={idx}")
    ...
    if active_a.count("ACTIVE ORDER: A THEN B") != 1:
        raise RuntimeError(f"V25_ACTIVE_TEMPLATE_MARKER_GUARD_FAILED pair={idx}")
    ...
    if archived_a.count("ARCHIVED ORDER: A THEN B") != 1:
        raise RuntimeError(f"V25_ARCHIVED_TEMPLATE_MARKER_GUARD_FAILED pair={idx}")
...
def validate_bridge_info(info: dict[str, Any]) -> None:
    checks = {
        ...
        "device_name": str(info.get("device_name", "")) == V26_EXPECTED_DEVICE_NAME,
        ...
        "torch": info.get("torch_version") == V26_EXPECTED_TORCH,
...
def run_discovery(client: ClientProtocol, output: Path) -> str:
    ...
    if len(competent_indices) < 12:
        result = {"kind":"PLANCARRY_LATENT_V2_6_DISCOVERY", "status":"INCONCLUSIVE_MODEL_EXPRESSIVITY",
                  "competent_count":len(competent_indices), "competence":competence,
                  "frozen_refs":{"experiment_id":EXPERIMENT_ID,"prediction_id":PREDICTION_ID,"frozen_design_sha256":DESIGN_SHA256,"manifest_sha256":MANIFEST_SHA256,"donor_map_sha256":DONOR_MAP_SHA256,"independent_review_sha256":REVIEW_SHA256,"registration_wrapper_sha256":REGISTRATION_WRAPPER_SHA256,"registration_wrapper_audit_sha256":REGISTRATION_WRAPPER_AUDIT_SHA256,"supersession_audit_sha256":SUPERSESSION_AUDIT_SHA256,"direct_vs_cache_diagnostic_sha256":DIRECT_VS_CACHE_SHA256,"prefix_stable_bridge_sha256":PREFIX_STABLE_BRIDGE_SHA256,"whitebox_client_sha256":WHITEBOX_CLIENT_SHA256,"prefix_stable_engineering_result_id":PREFIX_STABLE_ENGINEERING_RESULT_ID,"v26_runtime_rebind_sha256":V26_RUNTIME_REBIND_SHA256,"v26_runtime_rebind_review_sha256":V26_RUNTIME_REBIND_REVIEW_SHA256,"v26_registration_wrapper_sha256":V26_REGISTRATION_WRAPPER_SHA256,"v26_registration_wrapper_audit_sha256":V26_REGISTRATION_WRAPPER_AUDIT_SHA256},
    ...
    selection={
        "kind":"PLANCARRY_LATENT_V2_6_DISCOVERY_SELECTION",
...
def run_confirmation(client: ClientProtocol, selection_path: Path, selection_sha256: str, output: Path) -> str:
    ...
    if overall<16 or delayed<8:
        result={"kind":"PLANCARRY_LATENT_V2_6_CONFIRMATION","status":"INCONCLUSIVE_MODEL_EXPRESSIVITY",
    ...
    result={
        "kind":"PLANCARRY_LATENT_V2_6_CONFIRMATION","status":decision["status"],
...
def main() -> int:
    ...
    if out.exists():
        raise RuntimeError(f"REFUSE_EXISTING_OUTPUT {out}")
