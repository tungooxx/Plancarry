...
PRODUCER_SHA='bb05eb8b3b02f15d32f768212730712f2f0a04062729a57ca4993be2031dec55'
SANITY_PROTOCOL_SHA='9af0d247e8bb9cb5e17d11727008d827dab0c088d5c28142726be20cd2d883ef'
ALFWORLD_RUNTIME_SHA='53e550f70711a3779409c565ecbd3e2fd971751a03633dad3566d5569a6fb3c6'
TEXTWORLD_COMPAT_SHA='9506534d044fafff43e19db99cfaf93e808d670eb05572c8b338dd137b095da4'
DRIVER_KIND='PLANCARRY_REPLAYRESIDUAL_T1_DIRECT_OVERRIDE_CAUSAL_DEV_V1_2_PROVENANCE_HARDENED'
LAYERS=(7,14,21,27)
ALPHAS=(0.25,0.5,1.0)
...
def rescale_to(v:Any,target:float):
    ...
    # Frozen prereg norm rule applies to NONZERO controls. An exactly-zero
    # semantic control remains exact zero and is a valid intervention arm; it
    # must not delete/abort an otherwise qualified family.
    if n<=1e-12: return torch.zeros_like(x), True
    y=x*(float(target)/n)
...
def base_reset_provenance(tok:Any,packet:Mapping[str,Any]):
    ...
    try:
        ...
        reset_snapshot_sha256=sha_json({
            'world_state_sha256':state,
            'current_observation':obs,
            'admissible_actions':commands,
            'task_instruction':str(packet['task_instruction']).strip(),
            'reset_serialization':full,
        })
        return {
...
def engineering_sentinels(tok:Any,model:Any,packet:Mapping[str,Any],base:Mapping[str,Any]):
    ...
    for layer in LAYERS:
        # Keep only one KV session resident at a time: exact same comparison, bounded 4-GiB VRAM.
        base_s=PersistentTokenSession(model,base['prefix_ids'],layer=layer,vector=None)
        base_s.append_ids(base['action_prompt_ids'],event='ACTION_PROMPT_SENTINEL')
        baseline_logits=base_s.next_logits.detach().float().cpu().clone()
        base_s.close(); del base_s
        zero_s=PersistentTokenSession(model,base['prefix_ids'],layer=layer,vector=torch.zeros(hidden,dtype=torch.float32),scale=1.0)
        zero_s.append_ids(base['action_prompt_ids'],event='ACTION_PROMPT_SENTINEL')
        zmax=max(zmax,float(torch.max(torch.abs(zero_s.next_logits.detach().float().cpu()-baseline_logits)).item()))
        zero_s.close(); del zero_s
        ...
        self_s.append_ids(base['action_prompt_ids'],event='ACTION_PROMPT_SENTINEL')
        smax=max(smax,float(torch.max(torch.abs(self_s.next_logits.detach().float().cpu()-baseline_logits)).item()))
        self_s.close(); del self_s
...
def rollout(tok:Any,model:Any,packet:Mapping[str,Any],base:Mapping[str,Any],layer:int,vector:Any|None,alpha:float,arm:str,norm_guard:bool,active_residual_sha256:str):
    ...
    try:
        ...
        return {
            ...
            'arm_name':str(arm),'selected_layer':int(layer),'selected_alpha':float(alpha),
            'active_residual_sha256':str(active_residual_sha256),'injected_vector_sha256':prov['injected_vector_sha256'],
            'reset_snapshot_sha256':base['reset_snapshot_sha256'],
            ...
            'session_id_hash':prov['session_id_hash'],'session_runtime_layer':int(prov['layer']),'session_runtime_scale':float(prov['scale']),
...
def main()->int:
    ...
    required={
      ...
      ROOT/'replay_residual_natural_packet_producer_v2_1.py':PRODUCER_SHA,
      ROOT/'replay_residual_sanity_protocol_v1.py':SANITY_PROTOCOL_SHA,
      ROOT/'alfworld_runtime.py':ALFWORLD_RUNTIME_SHA,
      ROOT/'textworld_py313_compat.py':TEXTWORLD_COMPAT_SHA,
    }
    ...
    for pos,idx in enumerate(qualified,1):
        no_patch[idx]=rollout(tok,model,by_idx[idx],bases[idx],7,None,1.0,'NO_PATCH',False,source[idx][7]['active_sha256'])
    ...
    for layer in LAYERS:
        for alpha in ALPHAS:
            ...
            for pos,idx in enumerate(qualified,1):
                ...
                arms={
                    'ACTIVE_PLAN_RESIDUAL':rollout(tok,model,packet,bases[idx],layer,raw,alpha,'ACTIVE_PLAN_RESIDUAL',True,sv['active_sha256']),
                    # NO_PATCH behavior is deterministic and layer/alpha-independent; bind the
                    # reused baseline rollout to this candidate grid context while retaining
                    # its actual session_runtime_layer/session_runtime_scale fields.
                    'NO_PATCH':dict(no_patch[idx],arm_name='NO_PATCH',selected_layer=int(layer),selected_alpha=float(alpha),active_residual_sha256=sv['active_sha256']),
                    'RANDOM_EQ_NORM':rollout(tok,model,packet,bases[idx],layer,rand,alpha,'RANDOM_EQ_NORM',rand_guard,sv['active_sha256']),
                }
                for arm in ('NEXT_ACTION_PRESERVED_LATE_NULL','UNRELATED_PLAN','SHUFFLED_PLAN','GENERIC_HISTORY'):
                    if not guard[arm]: raise RuntimeError(f'CONTROL_RESIDUAL_NORM_GUARD_FAIL:{idx}:{layer}:{arm}')
                    arms[arm]=rollout(tok,model,packet,bases[idx],layer,controls[arm],alpha,arm,guard[arm],sv['active_sha256'])
