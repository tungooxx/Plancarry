...
class RealBackend:
    ...
    def __init__(self, model_id: str, revision: str, device: str, dtype: str, expected_device_substring: str):
        ...
        if device != "cuda":
            raise BridgeError("DEVICE_POLICY", "real PlanCarry bridge requires device='cuda'", 500)
    ...
    def _score_one(self, prompt: str, suffix: str, patch: dict[str, Any] | None = None) -> dict[str, Any]:
        prompt_ids, full_ids_cpu = self._continuation_ids(prompt, suffix)
        prompt_len = int(prompt_ids.numel())
        full_ids = full_ids_cpu.unsqueeze(0).to(self.device)
        ...
        try:
            with self.torch.inference_mode():
                out = self.model(input_ids=full_ids, attention_mask=attention_mask, use_cache=False)
                logits = out.logits[:, :-1, :].float()
                labels = full_ids[:, 1:]
                logp = self.torch.log_softmax(logits, dim=-1)
                tok_lp = logp.gather(-1, labels.unsqueeze(-1)).squeeze(-1)[0]
                # Token labels at positions prompt_len..end correspond to predictive logits indices prompt_len-1..end-2.
                cont_lp = tok_lp[prompt_len - 1 :]
                score = float(cont_lp.sum().item())
                token_count = int(cont_lp.numel())
        finally:
            for handle in handles:
                handle.remove()
        return {"suffix": suffix, "logprob_sum": score, "token_count": token_count, "patch_token_index_resolved": resolved_patch_idx}
