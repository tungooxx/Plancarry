#!/usr/bin/env python3
"""Frozen PlanCarry-Latent v2.2 discovery/confirmation runner.
...
live bridge calls require exact RTX3050/model provenance.
"""
...
from plancarry_latent_v2_2_validator import (
    DONOR_MAP_SHA256,
    MANIFEST_SHA256,
    PREREG_SHA256,
    REVIEW_SHA256,
...
def build_pair_context(pair: dict[str, Any]) -> PairContext:
    ...
    archived_prefix = prefix + "\nARCHIVED ORDER: "
    archived_a = archived_prefix + "A THEN B\nACTIVE ORDER: NONE\n<STATE_END>"
    archived_b = archived_prefix + "B THEN A\nACTIVE ORDER: NONE\n<STATE_END>"
...
def validate_bridge_info(info: dict[str, Any]) -> None:
    checks = {
        ...
        "device_name": "rtx 3050" in str(info.get("device_name", "")).lower(),
...
def _contexts_for_split(manifest: dict[str, Any], split: str) -> dict[int, PairContext]:
    ...
    for idx in wanted:
        ...
        out[idx] = build_pair_context(row)
    return out
...
def run_discovery(client: ClientProtocol, output: Path) -> str:
    ...
    contexts = _contexts_for_split(bundle["manifest"], "discovery")
    ...
    if len(competent_indices) < 12:
        result = {"kind":"PLANCARRY_LATENT_V2_2_DISCOVERY", "status":"INCONCLUSIVE_MODEL_EXPRESSIVITY",
...
def run_confirmation(client: ClientProtocol, selection_path: Path, selection_sha256: str, output: Path) -> str:
    ...
    contexts=_contexts_for_split(bundle["manifest"],"confirmation")
    competence={idx:source_competence(client,ctx) for idx,ctx in contexts.items()}
    ...
    if overall<16 or delayed<8:
        result={"kind":"PLANCARRY_LATENT_V2_2_CONFIRMATION","status":"INCONCLUSIVE_MODEL_EXPRESSIVITY",
    ...
    result={
        "kind":"PLANCARRY_LATENT_V2_2_CONFIRMATION","status":decision["status"],
...
def main() -> int:
    ...
    for name in ["discovery","confirmation"]:
        ...
        p.add_argument("--base-url",default="http://192.168.1.51:8765")
