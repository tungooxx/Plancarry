...
verify_exec_binding() {
  ...
  : "${ACTION_MATCHED_GROUNDED_V2_OPT_DRIVER_SHA256:?driver sha required}"
  : "${ACTION_MATCHED_GROUNDED_V2_OPT_RUNTIME_SHA256:?runtime sha required}"
  ...
  : "${ACTION_MATCHED_GROUNDED_V2_OPTIMIZATION_SHA256:?optimization sha required}"
  test "$(git rev-parse HEAD)" = "$ACTION_MATCHED_GROUNDED_V2_EXPECTED_GIT_COMMIT" || { echo 'git commit drift' >&2; exit 65; }
  test "$(sha256sum action_matched_grounded_v2_optimized_science_driver_v1.py | awk '{print $1}')" = "$ACTION_MATCHED_GROUNDED_V2_OPT_DRIVER_SHA256" || { echo 'driver hash drift' >&2; exit 66; }
  test "$(sha256sum action_matched_grounded_v2_optimized_runtime_v1.py | awk '{print $1}')" = "$ACTION_MATCHED_GROUNDED_V2_OPT_RUNTIME_SHA256" || { echo 'runtime hash drift' >&2; exit 67; }
  test "$(sha256sum action_matched_grounded_v2_optimization_v1.py | awk '{print $1}')" = "$ACTION_MATCHED_GROUNDED_V2_OPTIMIZATION_SHA256" || { echo 'optimization hash drift' >&2; exit 71; }
...
case "$PHASE" in
  preflight) verify_exec_binding; exec "$PY" action_matched_grounded_v2_optimized_science_driver_v1.py --phase preflight ;;
  development)
    ...
    verify_exec_binding
    exec "$PY" action_matched_grounded_v2_optimized_science_driver_v1.py --phase development --expected-device "$DEVICE" ;;
  confirmation)
    ...
    verify_exec_binding
    exec "$PY" action_matched_grounded_v2_optimized_science_driver_v1.py --phase confirmation --expected-device "$DEVICE" --development-seal-file-sha256 "$ACTION_MATCHED_GROUNDED_V2_DEVELOPMENT_SEAL_FILE_SHA256" ;;
