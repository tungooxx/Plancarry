...
SPLIT = "train"
MANIFEST_PATH = Path("/workspace/local-vlm/LLM/plancarry/results/design/gdaa_train_candidate_manifest_disjoint_v1.json")
EXPECTED_MANIFEST_SHA256 = "5beaeb849cc6abbee4397c1f8d5021700e272b799e356a2d77d47f920af4d418"
...
EXPECTED_DISJOINT_SOURCE_POOL = 610
...
def validate_candidate_isolation(manifest_path: Path = MANIFEST_PATH) -> dict[str, Any]:
    ...
    remaining = sorted(set(population) - set(binding))
    if len(remaining) != EXPECTED_DISJOINT_SOURCE_POOL:
        raise RuntimeError(
            f"disjoint source pool drift: {len(remaining)} != {EXPECTED_DISJOINT_SOURCE_POOL}"
        )
    expected_manifest_games = remaining[:MAX_GAMES_SCANNED]
    if manifest_games != expected_manifest_games:
        raise RuntimeError("frozen GDAA manifest does not equal first 90 lexicographic train paths outside binding pool")
    overlap = sorted(set(manifest_games) & set(binding))
    if overlap:
        raise RuntimeError(f"GDAA/binding candidate overlap must be zero; found {len(overlap)}")
    return {
        ...
        "disjoint_source_pool_count": len(remaining),
...
def run(output: Path, model: str = MODEL, base_url: str = BASE_URL) -> dict[str, Any]:
    ...
    result = {
        ...
        "candidate_order": "frozen_manifest_lexicographic_train_first_90",
