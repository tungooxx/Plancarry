#!/usr/bin/env python3
"""Pure validation/statistics helpers for reviewed PlanCarry-Latent v2.5.
...
EXPERIMENT_ID = "7d53ac0d-8ed4-4cf1-98b8-afd02fc794e9"
PREDICTION_ID = "77a66100-358e-4387-9cd9-4f0db8dfb14e"
DESIGN_SHA256 = "9dc788dcb3ec1e8646bff5526020df4913315b4f2a61059f67f332137b577b90"
...
REVIEW_SHA256 = "7b17804ed2ed5aa1b5e5524f7dbd274bf5b27c7e0b8b48ae3f57c8a112335ea5"
REGISTRATION_WRAPPER_SHA256 = "4baeb09009e66574ffa881e12ea8d27c2386dbe54ce3cb07c2c150f080213f9b"
REGISTRATION_WRAPPER_AUDIT_SHA256 = "90453ef592502e51c5b4efb84d5b35bd9f51c856fb3136030246bc32d91a733d"
SUPERSESSION_AUDIT_SHA256 = "78bb9da4167a7c46b269bdaa07aa3ca43a7efc63124a81dce15877a9abc37603"
...
def require_frozen_bundle(root: str|Path=".")->dict[str,Any]:
    root=Path(root)
    paths={
      "prereg":root/"results/design/plancarry_latent_v2_5_frozen_design_20260820T0244Z.json",
      ...
      "review":root/"results/design/plancarry_latent_v2_5_independent_prereg_review_20260820T0250Z.json",
      "registration_wrapper":root/"results/design/plancarry_latent_v2_5_registration_wrapper_20260820T0251Z.json",
      "registration_wrapper_audit":root/"results/design/plancarry_latent_v2_5_registration_wrapper_audit_20260820T0251Z.json",
      "supersession_audit":root/"results/design/plancarry_latent_v2_5_supersession_audit_20260820T0244Z.json",
    ...
    if prereg.get("scientific_result")!="NOT_ASSESSED" or integ.get("scientific_outcomes_seen") is not False or integ.get("confirmation_accessed") is not False:
        raise ValueError("V25_DESIGN_NOT_PROSPECTIVE_UNASSESSED")
    if prereg.get("model",{}).get("prefix_stable_bridge_sha256")!=PREFIX_STABLE_BRIDGE_SHA256 or prereg.get("model",{}).get("whitebox_client_sha256")!=WHITEBOX_CLIENT_SHA256:
        raise ValueError("V25_SCORER_PROVENANCE_MISMATCH")
    review=bundle["review"]
    if review.get("verdict")!="PASS_FOR_V25_ENGINEERING" or review.get("scientific_result")!="NOT_ASSESSED" or review.get("failed_checks")!=[] or not all(review.get("checks",{}).values()):
        raise ValueError("V25_INDEPENDENT_REVIEW_NOT_PASS")
    if review.get("design_sha256")!=DESIGN_SHA256 or review.get("direct_vs_cache_diagnostic_sha256")!=DIRECT_VS_CACHE_SHA256:
        raise ValueError("V25_REVIEW_PROVENANCE_MISMATCH")
    sup=bundle["supersession_audit"]
    if sup.get("scientific_result")!="NOT_ASSESSED" or sup.get("confirmation_accessed") is not False or sup.get("v2_5_design_sha256")!=DESIGN_SHA256:
        raise ValueError("V25_SUPERSESSION_AUDIT_INVALID")
    if not all(sup.get("held_exact_checks",{}).values()) or not all(sup.get("held_exact_model_scientific_fields",{}).values()):
        raise ValueError("V25_HELD_FIXED_AUDIT_FAILED")
    diag=bundle["direct_vs_cache"]
    if diag.get("scientific_result")!="NOT_ASSESSED" or diag.get("alfworld_science") is not False:
        raise ValueError("V25_DIRECT_CACHE_DIAGNOSTIC_SCOPE_INVALID")
    if abs(float(diag.get("max_abs_meanlp_delta"))-0.0016210079193115234)>1e-15:
        raise ValueError("V25_DIRECT_CACHE_DIAGNOSTIC_VALUE_MISMATCH")
    wrapper=bundle["registration_wrapper"]
    prov=wrapper.get("canonical_review_provenance",{})
    integ_w=wrapper.get("artifact_integrity",{})
    if prov.get("authoritative_design_sha256")!=DESIGN_SHA256 or prov.get("independent_review_sha256")!=REVIEW_SHA256 or prov.get("verdict")!="PASS_FOR_V25_ENGINEERING":
        raise ValueError("V25_REGISTRATION_WRAPPER_PROVENANCE_INVALID")
    if integ_w.get("scientific_outcomes_seen") is not False or integ_w.get("confirmation_accessed") is not False or integ_w.get("valid_seen_consumed") is not False or integ_w.get("valid_unseen_consumed") is not False:
        raise ValueError("V25_REGISTRATION_WRAPPER_NOT_PROSPECTIVE")
    wrapper_audit=bundle["registration_wrapper_audit"]
    if wrapper_audit.get("scientific_result")!="NOT_ASSESSED" or wrapper_audit.get("authoritative_design_sha256")!=DESIGN_SHA256 or wrapper_audit.get("registration_wrapper_sha256")!=REGISTRATION_WRAPPER_SHA256 or wrapper_audit.get("source_keys_exactly_unchanged") is not True:
        raise ValueError("V25_REGISTRATION_WRAPPER_AUDIT_INVALID")
    alias=wrapper_audit.get("alias_derivation",{})
    if not alias or not all(alias.values()):
        raise ValueError("V25_REGISTRATION_ALIAS_DERIVATION_FAILED")
    return bundle
...
def verify_selection_artifact(path: str|Path, expected_sha256: str)->dict[str,Any]:
    ...
    if obj.get("kind")!="PLANCARRY_LATENT_V2_5_DISCOVERY_SELECTION": raise ValueError("SELECTION_KIND_MISMATCH")
