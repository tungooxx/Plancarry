...
import planunique_fast_runtime_v1 as fast_runtime

ROOT=Path(__file__).resolve().parent
FAST_RUNTIME_SHA256='7e64be04d92d8a8e88ff315cb5ef2db69eb6d2019fbd593e89bf9c739c47f492'
...
def verify_sources()->dict[str,Any]:
 ...
 if sha_file(ROOT/'planunique_fast_runtime_v1.py')!=FAST_RUNTIME_SHA256:raise ExecutionContractError('PLANUNIQUE_FAST_RUNTIME_DRIFT')
...
def _run_msa(tok:Any,model:Any,p:Mapping[str,Any],base:Mapping[str,Any],layer:int,alpha:float,vpack:Mapping[str,Any],no_patch_cache:dict[tuple[int,int],dict[str,Any]]|None=None)->dict[str,Any]:
 ...
 for arm in (ACTIVE,NO_PATCH,*SPEC):
  if arm==NO_PATCH and no_patch_cache is not None and cache_key in no_patch_cache:
   ...
   row=fast_runtime.msa2_arm(tok,model,p,base,layer,None if arm==NO_PATCH else vectors[arm],alpha,arm,asha);arms[arm]=row
...
def confirmation(tok:Any,model:Any,prov:Mapping[str,Any])->dict[str,Any]:
 ...
 for i in phase.CONF:
  ...
  for arm in (ACTIVE,NO_PATCH,*SPEC):outs[arm]=fast_runtime.autonomous_arm(tok,model,by[i],base,layer,None if arm==NO_PATCH else vp['vectors'][arm],alpha,arm,asha)
